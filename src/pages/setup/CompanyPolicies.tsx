...
export default function CompanyPolicies() {
  ...
  return (
    <div className="p-6">
      <h2 className="text-xl font-semibold text-navy-900">Company Policies</h2>
      ...
      <div className="mb-4 flex items-end justify-between gap-4">
        <div className="max-w-xs flex-1">
          <label className="mb-1.5 block text-xs font-semibold text-gray-700">Company</label>
          <select className={FIELD_CLASS} value={companyId} onChange={(e) => setCompanyId(e.target.value)}>
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-100">
            <tr>
              <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">Branch</th>
              <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">User</th>
              <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">Role</th>
              <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">Policy type</th>
              <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">Value</th>
              <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">Allow</th>
            ...
            {loading ? (
              ...
              rules.map((r) => (
                <tr key={r.id} className="transition-colors hover:bg-brand-50">
                  <td className="px-3 py-2.5 text-gray-700">{r.branch?.name ?? "All branches"}</td>
                  <td className="px-3 py-2.5 text-gray-700">{r.user?.displayName ?? "All users"}</td>
                  <td className="px-3 py-2.5 text-gray-700">{r.role?.name ?? "All roles"}</td>
                  <td className="px-3 py-2.5 font-medium text-navy-900">{policyLabel(r.policyType)}</td>
                  <td className="px-3 py-2.5 text-gray-700">{r.value ?? "-"}</td>
                  <td className="px-3 py-2.5">
                    <span
                      className={`rounded-full border px-2.5 py-0.5 text-[11px] font-medium ${
                        r.allow ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-red-200 bg-red-50 text-red-600"
                      }`}
                    >
                      {r.allow ? "Yes" : "No"}
                    </span>
